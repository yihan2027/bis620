...
[![Codecov test coverage](https://codecov.io/gh/yihan2027/bis620/branch/main/graph/badge.svg)](https://app.codecov.io/gh/yihan2027/bis620?branch=main)
<!-- badges: end -->

Here is the [lint result.](https://github.com/yihan2027/bis620/actions/workflows/lint.yaml)

The goal of bis620.2022 is to visualize UKBiobank accelerometry data by time series plot and spectral signature plot.
...
This is a basic example of using `accel_plot()` to draw a time-series plot of `X`, `Y` and `Z` using UKBiobank accelerometry data.
...
This is a basic example of using `spectral_sig()` to get the spectral signature of accelerometry data.  
The parameter `take_log` decides whether the log of the modulus should be taken. (Default is `FALSE`)  
The parameter `inverse` decides whether the unnormalized inverse transform is computed. (Default is `TRUE`)
